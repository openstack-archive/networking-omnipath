#!/bin/sh
# This script has been taken from neutron repo

set -eu

usage () {
    echo "Usage: $0 [OPTION]..."
    echo "Run networking-omnipath's coding check(s)"
    echo ""
    echo "  -Y, --pylint [<basecommit>] Run pylint check on the entire networking-omnipath module or just files changed in basecommit (e.g. HEAD~1)"
